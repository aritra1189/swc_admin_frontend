// src/pages/AdminStreamManagement.jsx
...
export default function AdminStreamManagement() {
  ...
  // PATCH: Update Stream
  const updateStream = async (id, name) => {
    try {
      await axios.patch(
  ...
  const updateStreamStatus = async (id, currentStatus) => {
    const newStatus = currentStatus === "ACTIVE" ? "DEACTIVE" : "ACTIVE";
    try {
      await axios.put(
        `${API_BASE_URL}/stream/status/${id}`,
        { status: newStatus },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  // GET: Fetch connected semesters for a stream
  const fetchConnectedSemesters = async (streamId) => {
    try {
      setLoadingConnections(true);
      const response = await axios.get(`${API_BASE_URL}/stream-sem/${streamId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  // POST: Connect semesters to stream
  const connectSemestersToStream = async (streamId, semesterIds) => {
    try {
      const promises = semesterIds.map(semesterId => 
        axios.post(`${API_BASE_URL}/stream-sem`, {
          streamId,
          semesterId
        }, {
          headers: { Authorization: `Bearer ${token}` }
      ...
      fetchConnectedSemesters(streamId);
    } catch (error) {
  ...
  const removeSemesterConnection = async (connectionId, streamId) => {
    try {
      await axios.delete(`${API_BASE_URL}/stream-sem/remove/${connectionId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        await updateStream(editId, name);
        setEditId(null);
      } else {
  ...
  const handleOpenSemesterModal = async (stream) => {
    setSelectedStream(stream);
    await fetchAllSemesters();
    await fetchConnectedSemesters(stream.id);
  ...
  const handleConnectSemesters = async () => {
    ...
    await connectSemestersToStream(selectedStream.id, selectedSemesters);
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-md mb-8">
        <h2 className="text-xl font-semibold mb-4">
          {editId ? "Edit Stream" : "Create New Stream"}
        </h2>
        ...
        <div className="mt-6 flex justify-end space-x-4">
          ...
          >
            {editId ? "Update Stream" : "Create Stream"}
          </button>
        </div>
      </form>
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        <h2 className="text-xl font-semibold mb-4">Streams List</h2>
        
        {loading ? (
          <div className="text-center py-8">Loading streams...</div>
        ) : streams.length === 0 ? (
          <div className="text-center py-8">No streams found</div>
        ) : (
          <>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {streams.map((stream, index) => (
                    <tr key={stream.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap font-medium">{stream.name}</td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          ...
                          onClick={() => handleStatusToggle(stream.id, stream.status)}
                        >
                          {stream.status}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap space-x-2">
                        <div className="flex flex-wrap gap-2">
                          ...
                          <button
                            onClick={() => handleOpenSemesterModal(stream)}
            ...
            <div className="flex justify-between items-center mt-4">
              <div>
                Showing {pagination.offset + 1} to {Math.min(pagination.offset + pagination.limit, pagination.total)} of {pagination.total} streams
              </div>
            ...
            <div className="p-6">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-semibold">
                  Manage Semesters for Stream: {selectedStream?.name}
                </h2>
                <button
              ...
              <div className="mb-6">
                ...
                {loadingConnections ? (
                  ...
                  <p className="text-gray-500">No semesters connected to this stream</p>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    {connectedSemesters.map((connection) => (
                      <div key={connection.id} className="border rounded p-3 flex justify-between items-center">
                        ...
                        <button
                          onClick={() => removeSemesterConnection(connection.id, selectedStream.id)}
